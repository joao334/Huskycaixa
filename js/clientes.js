(() => {
  ...
  const CLIENTS_PAGE = {
    ...
    init() {
      ...
      this.renderAll();
      app.log('Tela de clientes carregada.');
    },
    ...
    resetForm(skipScroll = false) {
      ...
      this.refs.clientCode.value = app.createId('CLI');
    ...
    validateForm() {
      ...
      if (!phone && !email) {
        app.showToast('Informe ao menos telefone ou e-mail do cliente.', 'warning');
        this.refs.clientPhone.focus();
        return false;
      }

      const duplicatePhone = phone && this.getClients().find((client) => {
        return client.id !== this.editingClientId && this.normalizePhone(client.phone) === this.normalizePhone(phone);
      });

      if (duplicatePhone) {
        app.showToast('Já existe um cliente com esse telefone.', 'warning');
        return false;
      }

      const duplicateEmail = email && this.getClients().find((client) => {
        return client.id !== this.editingClientId && String(client.email || '').toLowerCase() === email.toLowerCase();
      });

      if (duplicateEmail) {
        app.showToast('Já existe um cliente com esse e-mail.', 'warning');
        return false;
      }
    ...
    buildClientPayload() {
      ...
      return {
        id: existing?.id || this.refs.clientId.value || crypto.randomUUID(),
        code: existing?.code || this.refs.clientCode.value || app.createId('CLI'),
    ...
    handleSaveClient(isUpdate = false) {
      ...
      state.customers = app.upsertItem(state.customers || [], client, 'id');
      this.setState(state);
      this.selectedClientId = client.id;
      this.renderAll();
      app.showToast(isUpdate || this.editingClientId ? 'Cliente atualizado com sucesso.' : 'Cliente salvo com sucesso.', 'success');
      app.log('Cliente salvo/atualizado.', { clientId: client.id, code: client.code, name: client.name });
      this.resetForm();
    },
    ...
    handleDeleteClient() {
      ...
      if (!client) {
        app.showToast('Selecione um cliente para excluir.', 'warning');
        return;
      }

      const clientSales = this.getSalesByClient(client);
      if (clientSales.length) {
        app.showToast('Este cliente possui histórico de vendas e não pode ser excluído. Deixe-o inativo.', 'warning');
        return;
      }

      const confirmed = app.confirmAction(`Deseja excluir o cliente ${client.name}?`);
      if (!confirmed) return;

      const state = this.getState();
      state.customers = app.removeById(state.customers || [], client.id);
      this.setState(state);
      ...
      app.log('Cliente excluído.', { clientId: client.id, code: client.code, name: client.name });
      this.resetForm();
    },
    ...
    updateLiveSummary() {
      ...
      this.refs.clientStatusBirthday.textContent = clientDraft.birthday ? app.formatDate(clientDraft.birthday) : '-';
      ...
      this.refs.clientTotalSpent.textContent = app.formatCurrency(history.totalSpent || 0);
      this.refs.clientLastOrder.textContent = history.lastOrderDate ? app.formatDate(history.lastOrderDate) : '-';
      this.refs.clientAverageTicket.textContent = app.formatCurrency(history.averageTicket || 0);
    ...
    renderMetrics() {
      ...
      this.refs.clientsBestCustomer.textContent = bestCustomer
        ? `${bestCustomer.name} • ${app.formatCurrency(bestCustomerStats.totalSpent)}`
        : '-';
    },
      ...
      this.refs.clientsTableBody.innerHTML = clients.map((client) => {
        ...
        return `
          <tr>
            ...
            <td>${stats.lastOrderDate ? app.formatDate(stats.lastOrderDate) : '-'}</td>
            <td>${app.formatCurrency(stats.totalSpent || 0)}</td>
            <td>
    ...
    renderSelectedClientPanels() {
      ...
      this.refs.clientTotalSpent.textContent = app.formatCurrency(stats.totalSpent || 0);
      this.refs.clientLastOrder.textContent = stats.lastOrderDate ? app.formatDate(stats.lastOrderDate) : '-';
      this.refs.clientAverageTicket.textContent = app.formatCurrency(stats.averageTicket || 0);
    ...
    renderFeaturedClients() {
      const featured = this.getClients()
        ...
        .sort((a, b) => {
          const aScore = (a.client.status === 'VIP' ? 1000000 : 0) + (a.client.priorityService ? 500000 : 0) + a.stats.totalSpent;
          const bScore = (b.client.status === 'VIP' ? 1000000 : 0) + (b.client.priorityService ? 500000 : 0) + b.stats.totalSpent;
      ...
      this.refs.featuredClientsList.innerHTML = featured.map(({ client, stats }) => `
        <div class="pix-proof-card">
          <div>
            ...
            <p>${stats.ordersCount} pedido(s) • ${app.formatCurrency(stats.totalSpent)} acumulados</p>
          </div>
    ...
    renderBirthdays() {
      ...
      this.refs.clientsBirthdayTableBody.innerHTML = birthdays.map(({ client, distance }) => `
        <tr>
          <td>${this.escapeHtml(client.name)}</td>
          <td>${client.birthday ? app.formatDate(client.birthday) : '-'}</td>
          <td>${this.escapeHtml(client.phone || client.email || '-')}</td>
          <td><button type="button" class="btn btn-secondary btn-small" data-action="select-client" data-id="${client.id}">${distance <= 7 ? 'Priorizar' : 'Marcar'}</button></td>
        </tr>
      `).join('');
    },
    ...
    handleTableActions(event) {
      ...
      if (action === 'view-client') {
        ...
        this.renderSelectedClientPanels();
        window.scrollTo({ top: document.querySelector('.content-grid.two-columns')?.offsetTop || 0, behavior: 'smooth' });
      }
    },
    ...
    handleBirthdayActions(event) {
      ...
      this.renderSelectedClientPanels();
      app.showToast('Cliente selecionado para acompanhamento.', 'success');
    },

    loadClientIntoForm(clientId) {
      const client = this.findClientById(clientId);
      if (!client) {
        app.showToast('Cliente não encontrado.', 'danger');
        return;
      }
    ...
    getClientStats(client, isDraft = false) {
      ...
      const totalSpent = app.sum(sales, (sale) => sale.total || 0);
    ...
    matchSalesToDraftClient(draftClient) {
      const draftName = app.normalizeText(draftClient.name || '');
      ...
      return this.getSales()
        .filter((sale) => sale.orderStatus !== 'Cancelado')
        .filter((sale) => {
          const saleName = app.normalizeText(sale.client?.name || '');
    ...
    saleMatchesClient(sale, client) {
      ...
      const saleEmail = String(sale.client?.email || '').toLowerCase().trim();
      const clientName = app.normalizeText(client.name || '');
      const saleName = app.normalizeText(sale.client?.name || '');
    ...
    isWithoutRecentPurchase(lastOrderDate) {
      ...
      const today = new Date(app.todayISO());
